...
```{r scatter-body-energy-season}

body_energy_season <- heatwaves_coral_fish_clean |>
  ggplot(aes(x = total_length, y = SDA_integrated, color = season)) +
  geom_point(alpha = 0.8, size = 3) +
  labs(
    title = "Body Length vs. SDA (by Season)",
    subtitle = "SDA stands for specific dynamic action",
    x = "Total Length (cm)",
    y = "SDA Integrated"
  )

# Save the plot
ggsave("fig_body_energy_season_scatterplot.png", body_energy_season, width = 7, height = 5)
```
